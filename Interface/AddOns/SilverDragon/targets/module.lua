...
function module:OnInitialize()
	...
	if config then
		local GetLocaleText = config.GetLocaleText
		config.options.plugins.clicktarget = {
			clicktarget = {
				type = "group",
				name = GetLocaleText("ClickTarget"),
				...
				args = {
					...
					closeAfter = {
						type = "range",
						name = GetLocaleText("Close after"),
					...
					sources = {
						type="multiselect",
						name = GetLocaleText("Rare Sources"),
						desc = GetLocaleText("Which ways of finding a rare should cause this frame to appear?"),
						get = function(info, key) return self.db.profile.sources[key] end,
						set = function(info, key, v) self.db.profile.sources[key] = v end,
						values = {
							target = GetLocaleText("Targets"),
							grouptarget = GetLocaleText("Group targets"),
							mouseover = GetLocaleText("Mouseover"),
							nameplate = GetLocaleText("Nameplates"),
							vignette = GetLocaleText("Vignettes"),
							['point-of-interest'] = GetLocaleText("Map Points of Interest"),
							groupsync = GetLocaleText("Group Sync"),
							guildsync = GetLocaleText("Guild Sync"),
						},
					},
					style = {
						type = "select",
						name = GetLocaleText("Style"),
