...
WebDKP_Roll_Total = 1;
defaultBIPmsg = "WebDKP: $name 以 $dkp 的高价获得竞拍. 还有 $time 秒可以竞拍 $item!"
defaultSBIPmsg = "WebDKP: 还有 $time 秒可以竞拍 $item!"
defaultRIPmsg = "WebDKP: $name 以 $roll 点，为最高Roll点. 还有 $time 秒可以Roll $item"
...
function WebDKP_Bid_Event(arg1, arg2)
    ...
    if (WebDKP_IsBidChat(name, trigger)) then
        ...
        if (string.find(string.lower(trigger), "出价 ") == 1) then
            if cmd == nil then
                WebDKP_SendWhisper(name, "你出价不当, 输入'出价 分数'");
            else
                if (WebDKP_bidInProgress == false) then
                    WebDKP_SendWhisper(name, "没有正在进行的竞拍");
                elseif (cmd == "" or cmd == nil) then
                    WebDKP_SendWhisper(name, "你没有指定竞拍金额");
                elseif ((cmd) < WebDKP_GetStartingBid()) then
                    WebDKP_SendWhisper(name, "竞拍不接受. 最低出价是 " .. WebDKP_GetStartingBid());
                elseif (WebDKP_Options["BidFixedBidding"] == 1) then
                    WebDKP_SendWhisper(name, "竞拍不接受. 你必须输入'需求'");
                elseif (WebDKP_Options["DisableBid"] == 1) then
                    WebDKP_SendWhisper(name, "竞拍不接受. 请输入 ‘主天赋’ 或者 ‘副天赋’");
                else
                    flag = WebDKP_Bid_HandleBid(name, cmd, "NA");
                    if flag == 1 then
                        WebDKP_SendWhisper(name, "竞拍： " .. cmd .. " dkp ，已接受");
                    end
                end
            end
            -- SOMEONE WANTS TO BID ALL THEIR DKP
        elseif (string.find(string.lower(trigger), "SH") == 1) then
            local dkp = WebDKP_GetDKP(name);
            if (WebDKP_bidInProgress == false) then
                WebDKP_SendWhisper(name, "没有正在进行的竞拍");
            elseif (dkp < WebDKP_GetStartingBid()) then
                WebDKP_SendWhisper(name, "竞拍不接受, 你没有足够的 DKP. 最低出价是 " .. WebDKP_GetStartingBid());
            elseif (WebDKP_Options["BidFixedBidding"] == 1) then
                WebDKP_SendWhisper(name, "竞拍不接受. 你必须输入'需求'");
            else
                flag = WebDKP_Bid_HandleBid(name, dkp, "NA");
                if flag == 1 then
                    WebDKP_SendWhisper(name, "竞拍： " .. dkp .. " dkp ，已接受");
                end
            end
            -- SOMEONE WANTS TO BID FOR MAIN SPEC
        elseif (string.find(string.lower(trigger), "主天赋") == 1) then
            ...
            elseif (cmd == "" or cmd == nil) then
                ...
                if flag == 1 then
                    if WebDKP_Options["BidFixedBidding"] == 1 then
                        WebDKP_SendWhisper(name, "你的主天赋需求已经被接受!");
                    else
                        WebDKP_SendWhisper(name, "主天赋竞拍: " .. cmd .. " dkp 被接受!");
                    end
                end
            elseif (cmd < WebDKP_GetStartingBid()) then
                WebDKP_SendWhisper(name, "竞拍不接受, 你没有足够的 DKP. 最低竞拍是: " .. WebDKP_GetStartingBid());
            elseif (WebDKP_Options["BidFixedBidding"] == 1) then
                WebDKP_SendWhisper(name, "竞拍不接受. 你必须输入'需求'");
            else
                flag = WebDKP_Bid_HandleBid(name, cmd, "Main");
        ...
        elseif (string.find(string.lower(trigger), "副天赋") == 1) then
            if (WebDKP_bidInProgress == false) then
                WebDKP_SendWhisper(name, "没有正在进行的竞拍");
            elseif (cmd == "" or cmd == nil) then
                cmd = 0;
                ...
                if WebDKP_Options["BidFixedBidding"] == 1 then
                    WebDKP_SendWhisper(name, "你的副天赋需求已被接受.");
                else
                    WebDKP_SendWhisper(name, "副天赋竞拍: " .. cmd .. " dkp 已被接受");
                end
            elseif (cmd < WebDKP_GetStartingBid()) then
                WebDKP_SendWhisper(name, "竞拍不接受, 你没有足够的 DKP. 最低竞拍是: " .. WebDKP_GetStartingBid());
            elseif (WebDKP_Options["BidFixedBidding"] == 1) then
                WebDKP_SendWhisper(name, "竞拍不接受. 你必须输入'贪婪'");
            else
                flag = WebDKP_Bid_HandleBid(name, cmd, "Off");
                if flag == 1 then
                    WebDKP_SendWhisper(name, "副天赋竞拍: " .. cmd .. " DKP 被接受");
                end
            end

            -- THEY WANT THE BIDDING TO START
        elseif (string.find(string.lower(trigger), "开拍") == 1) then
            if (WebDKP_bidInProgress == true) then
                WebDKP_SendWhisper(name, "已经有一个竞拍在进行中-你不能开始另一个竞拍");
            elseif (startbiddingcmd == "" or startbiddingcmd == nil) then
                WebDKP_SendWhisper(name, "必须指定要竞拍的物品. 如: 开拍 [龙牙]");
            else
        ...
        elseif (string.find(string.lower(trigger), "停拍") == 1) then
            if (WebDKP_bidInProgress == false) then
        ...
        elseif (string.find(string.lower(trigger), "需求") == 1 and WebDKP_Options["BidFixedBidding"] == 1) then
            dkp = 0;
            if WebDKP_Options["EPGPEnabled"] == 0 then
        ...
        elseif (string.find(string.lower(trigger), "贪婪") == 1 and WebDKP_Options["BidFixedBidding"] == 1) then
            if WebDKP_Options["EPGPEnabled"] == 0 then
                dkp = WebDKP_GetDKP(name); -- bid all their dkp. in a fixed bid they will only be charged the cost from the loot table - this is just for ordering
                if dkp == nil then
                    dkp = 0;
            ...
            if flag == 1 then
                WebDKP_SendWhisper(name, "副天赋竞拍: " .. dkp .. " dkp 被接受");
            end
...
function WebDKP_IsBidChat(name, trigger)
    if (string.find(string.lower(trigger), "出价") == 1 or
            string.find(string.lower(trigger), "SH") == 1 or
            string.find(string.lower(trigger), "主天赋") == 1 or
            string.find(string.lower(trigger), "副天赋") == 1 or
            string.find(string.lower(trigger), "开拍") == 1 or
            string.find(string.lower(trigger), "停拍") == 1 or
            string.find(string.lower(trigger), "需求") == 1 or
            string.find(string.lower(trigger), "贪婪") == 1
    ) then
        return true
    end
    return false
...
function WebDKP_Bid_StartBid(item, time)
    if item ~= "" and item ~= nil then
        ...
        if (WebDKP_Options["BidAnnounceRaid"] == 1) then
            WebDKP_SendAnnouncement("竞拍开始!", "RAID_WARNING");
        end
...
function WebDKP_Bid_HandleBid(playerName, bidAmount, spec)
    ...
    if (WebDKP_bidInProgress) then
        ...
        if (postDkp < 0 and WebDKP_Options["BidAllowNegativeBids"] == 0) then
            WebDKP_SendWhisper(playerName, "竞拍被拒绝 - 你不能出价超过你当前DKP.");
            WebDKP_SendWhisper(playerName, "你的最高出价是 " .. dkp);
        ...
        elseif (postDkp2 < 0 and WebDKP_Options["BidAllowNegativeBids"] == 0) then
            WebDKP_SendWhisper(playerName, "竞拍被拒绝 - 赢得此物品的最低dkp是 " .. startingBid);
        else
            ...
            WebDKP_SendWhisper(playerName, "收到竞拍!");
            ...
            if (highBidder == playerName and WebDKP_Options["BidNotifyLowBids"] == 1) then
                WebDKP_SendWhisper(playerName, "你是目前出价最高的人");
            elseif (highBidder ~= playerName and WebDKP_Options["BidNotifyLowBids"] == 1) then
                WebDKP_SendWhisper(playerName, "你不是出价高的人. 目前的最高价是  " .. highBid .. " dkp.");
            end

        end
        return flag;
    else
        WebDKP_SendWhisper(playerName, "没有正在进行的竞拍");
    end
end
    ...
    if (player == nil) then
        ...
        if (WebDKP_Options["BidConfirmPopup"] == 1 or bid == nil) then
            if (WebDKP_Options["BidFixedBidding"] == 1 and bid == nil) then
                WebDKP_Bid_ShowConfirmFrame("授予 " .. player .. " " .. link .. "? |cFFFF0000(物品不在战利品表中)|r", 0);
            elseif (WebDKP_Options["BidFixedBidding"] == 1 and WebDKP_Options["EPGPEnabled"] == 0) then
                WebDKP_Bid_ShowConfirmFrame("授予 " .. player .. " " .. link .. " ， " .. bid .. " dkp?(取自战利品列表)", bid);
            elseif (WebDKP_Options["EPGPEnabled"] == 1) then
                WebDKP_Bid_ShowConfirmFrame("授予 " .. player .. " " .. link .. " ， " .. bid .. " GP?", bid);
            else
                WebDKP_Bid_ShowConfirmFrame("授予 " .. player .. " " .. link .. " ， " .. bid .. " dkp? (百分比，如：50%)", bid);
            end
...
function WebDKP_Bid_OnUpdate(self, elapsed)
    ...
    if (this.TimeSinceLastUpdate > 1.0) then
        ...
        WebDKP_BidFrame_Countdown:SetText("剩余时间: " .. WebDKP_bidCountdown .. "秒");
...
function WebDKP_Roll_Start(item, time)
    ...
    if item ~= "" and item ~= nil then
        ...
        WebDKP_BidFrameRollButton:SetText("停止Roll点!");
        ...
        -- if the options ask for it, also make an announcement in a raid warning
        if (WebDKP_Options["BidAnnounceRaid"] == 1) then
            WebDKP_SendAnnouncement("Roll点开始!", "RAID_WARNING");
        end
...
function WebDKP_Roll_Stop()
    ...
    WebDKP_BidFrameRollButton:SetText("开始Roll点!");        -- fix the button text
