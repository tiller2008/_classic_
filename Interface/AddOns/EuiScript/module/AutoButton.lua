...
local function EuiIsUsableItem(itemId)
...
local function IsSlotItem(itemId)
	local itemSpell = EuiIsUsableItem(itemId)
...
--	GetWorldQuestItemList('init');
	...
	for k, v in pairs(QuestItemList) do
		if not db.blankList[k] and IsUsableItem(k) then
	...
	if db.questNum > 0 then
		...
		for i = 1, #questItemIDList do
			...
			AutoButton.spellName = EuiIsUsableItem(itemID)
	...
	if db.slotNum > 0 then
		for w = 1, 18 do
			...
			--	AutoButton.itemLink = GetInventoryItemLink('player', w)
				AutoButton.spellName = EuiIsUsableItem(slotID)
