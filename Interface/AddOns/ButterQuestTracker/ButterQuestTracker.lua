...
BQT:RegisterEvent("ADDON_LOADED", "OnPlayerEnteringWorld")
...
function BQT:GetTrackerHeader(visibleQuestCount, questCount)
	...
	local queststr = BQTL and BQTL:GetString('SETTINGS_TRACKER_HEADER_FORMAT_QUESTS_OPTION') or "Quests"

    if self.db.global.TrackerHeaderFormat == "QuestsNumberVisible" then
        return queststr.. " (" .. visibleQuestCount .. "/" .. questCount .. ")";
    elseif self.db.global.TrackerHeaderFormat == "QuestsNumberVisibleTotal" then
        return queststr.. " (" .. visibleQuestCount .. "/" .. C_QuestLog.GetMaxNumQuests() .. ")";
    end
