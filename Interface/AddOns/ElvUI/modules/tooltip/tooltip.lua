...
function TT:GameTooltip_SetDefaultAnchor(tt, parent)
	...
	if tt.StatusBar then
		if(parent) then
			local p1, _ = tt.StatusBar:GetPoint(); --by eui.cc fix
			if p1 == "TOPLEFT" then
				tt.StatusBar.anchoredToTop = false
...
--[[
	...
	if E.db.tooltip.range and UnitExists(unit) and unit ~= "player" and rc then
		...
		if maxRange then
			...
			tt:AddDoubleLine(L["Range"]..":", text)
		else
			tt:AddDoubleLine(L["Range"]..":", '80+')
		end
	end
...
function TT:SetStyle(tt)
	...
	if E.db.general.transparent and not self.db.transparent then
		tt:SetTemplate('notrans', nil, true) --ignore updates
	else
