...
local function GetTemplate(template, isUnitFrameElement)
	...
	if E.db.general.transparent and template ~= 'notrans' then
...
local function SetTemplate(frame, template, glossTex, ignoreUpdates, forcePixelMode, isUnitFrameElement)
	...
	if template == 'NoBackdrop' then
		...
		if E.db.general.transparent and template ~= 'notrans' then
			E:SetBackdrop(frame, true, E.media.blankTex, (not E.twoPixelsPlease and E.mult) or E.mult*2)
		else
		...
		if not frame.ignoreBackdropColors then
			if template == 'Transparent' or (E.db.general.transparent and template ~= 'notrans') then
		...
		if not E.PixelMode and not frame.forcePixelMode and not E.db.general.transparent then
...
local function CreateShadow(frame, size)
	...
	size = size or E.db.general.ShadowWidth
	if not size or size == 0 then size = 0.5 end
