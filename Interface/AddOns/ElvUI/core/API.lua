...
function E:IsDisabledAddon(name, skipDisabled)
	if skipDisabled then return false end
...
function E:IsConfigurableAddOn(name, skipDisabled)
	if E:IsEuiAddOn(name) and (not E:IsDisabledAddon(name, skipDisabled)) then
		return true;
	else
		return false;
	end
