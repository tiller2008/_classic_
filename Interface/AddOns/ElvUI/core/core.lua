...
--Constants
E.noop = function() end
E.title = format('|cfffe7b2c%s |r', 'EUI')
...
E.FullVersion = GetAddOnMetadata("EuiScript", "Version")
E.Ver = E.FullVersion or E.version
if E.FullVersion then E.Version = string.sub(E.FullVersion,2,9); end
...
function E:Print(...)
	(_G[self.db.general.messageRedirect] or _G.DEFAULT_CHAT_FRAME):AddMessage(strjoin('', self.media.hexvaluecolor or '|cff00b3ff', 'EUI:|r ', ...)) -- I put DEFAULT_CHAT_FRAME as a fail safe.
end
...
do
	...
	_G.C_ChatInfo.RegisterAddonMessagePrefix('EUI_VERSIONCHK')
...
--NamePlates:ConfigureAll()
	...
	if E.private.nameplates_v1.enable then
		NamePlates = E:GetModule('NamePlates_v1')
		NamePlates:ConfigureAll()
	--	NamePlates:StyleFilterInitializeAllFilters()
	elseif E.private.nameplates.enable then
		NamePlates:ConfigureAll()
	--	NamePlates:StyleFilterInitialize()
	end
...
--	end
	...
	if E.zhlocale then
		E:Print(L["捞月狗App 正式上线, 魔兽玩家聊天交友必备软件, 数据进度查询应有尽有.访问:http://www.laoyuegou.com"])
		...
		gga:SetScript("OnUpdate", function(self, elapsed)
			...
			if self.sec > 1234 then
				E:Print(L["捞月狗App 正式上线, 魔兽玩家聊天交友必备软件, 数据进度查询应有尽有.访问:http://laoyuegou.com"])
				self.sec = 0
			end
		end)
	end

	if E.db.general.transparent then E:UpdateBorderColors() end

	E:LoadSingleAddon() --载入单体插件
end
