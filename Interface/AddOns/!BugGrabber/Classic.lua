...
function GetSpellID(spellName, unit, auraType)
...
function GetSpecializationRole()
	...
	elseif (myclass == "PALADIN" and tree == 1) or (myclass == "DRUID" and tree == 3) or (myclass == "SHAMAN" and tree == 3) or (myclass == "PRIEST" and tree ~= 3) then
		...
		if myclass ~= "HUNTER" and heal >= ap and heal >= spell then
			return "HEALER" -- healing gear without having the majority of talents in a healing tree
		elseif myclass ~= "HUNTER" and (isCaster[myclass][tree] or spell >= ap) then
...
ThreatLib.RegisterCallback(ThreatFrame, "Activate", dummy)
ThreatLib.RegisterCallback(ThreatFrame, "Deactivate", dummy)
