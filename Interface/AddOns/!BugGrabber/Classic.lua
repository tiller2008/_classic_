...
local _, myclass, myClassID = UnitClass('player')

----------------------------------------------------------------------------------------
--	NOOP / Pass Functions not found in Classic
----------------------------------------------------------------------------------------
GetCurrencyInfo = _G.GetCurrencyInfo or dummy
GetCurrencyListInfo = _G.GetCurrencyListInfo or dummy
GetCurrencyListSize = _G.GetCurrencyListSize or function()
	return 0
end

GetContainerItemEquipmentSetInfo = _G.GetContainerItemEquipmentSetInfo or dummy
IsContainerItemAnUpgrade = _G.IsContainerItemAnUpgrade or dummy

GetProfessions = _G.GetProfessions or dummy

GetFriendshipReputation = _G.GetFriendshipReputation or dummy
IsXPUserDisabled = _G.IsXPUserDisabled or dummy
IsWatchingHonorAsXP = _G.IsWatchingHonorAsXP or dummy
IsInActiveWorldPVP = _G.IsInActiveWorldPVP or dummy

UnitInVehicle = _G.UnitInVehicle or dummy
UnitHasVehicleUI = _G.UnitHasVehicleUI or dummy
UnitIsWildBattlePet = _G.UnitIsWildBattlePet or dummy
UnitIsBattlePetCompanion = _G.UnitIsBattlePetCompanion or dummy

PlayerTalentFrame_Toggle = _G.PlayerTalentFrame_Toggle or dummy
...
GetSpecializationRole = _G.GetSpecializationRole or function()
	...
	local role
	if ((myclass == "PALADIN" and tree == 2) or (myclass == "WARRIOR" and tree == 3)) or (myclass == "DRUID" and tree == 2 and GetBonusBarOffset() == 3) then
		role = "TANK"
	elseif ((myclass == "PALADIN" and tree == 1) or (myclass == "DRUID" and tree == 3) or (myclass == "SHAMAN" and tree == 3) or (myclass == "PRIEST" and tree ~= 3)) then
		role = "HEALER"
	else
		...
		if (((ap > int) or (agi > int)) and not (myclass == "SHAMAN" and tree ~= 1 and tree ~= 3) and not AuraUtil.FindAuraByName(GetSpellInfo(24858), "player")) or myclass == "ROGUE" or myclass == "HUNTER" or (myclass == "SHAMAN" and tree == 2) then
...
GetSpecializationInfo = _G.GetSpecializationInfo or function(specIndex, ...)
	local id = specializationID[myclass][specIndex]
	local name, description, points, background = GetTalentTabInfo(specIndex)
	local icon = "Interface\\ICONS\\" .. specializationIcons[myclass][specIndex]
...
-- Add later
GetInspectSpecialization = _G.GetInspectSpecialization or dummy
...
UnitDetailedThreatSituation = ThreatLib and function(unit, mob)
